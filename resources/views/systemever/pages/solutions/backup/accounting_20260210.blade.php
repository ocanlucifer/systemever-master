    ...
    <div class="cloud-container">
        ...
        <div class="cloud-hero-grid">
            <div>
                ...
                <h1>{{ stringlang('Accounting System', 'Manajemen Akuntansi', '회계 관리 솔루션') }}</h1>
                <p>{{ stringlang('An accounting management solution that', 'Solusi manajemen akuntansi', '다차원 데이터 구조로 재무 현황을 직관적으로') }}
                    <br>{{ stringlang('provides a clear, at-a-glance view of', 'dengan struktur data multidimensi', '한눈에 보여주는 회계 관리 솔루션') }}
                    @if(activelang() != 'KOR')
                        <br>{{ stringlang('financial status with a multi-dimensional', 'yang menghadirkan visibilitas keuangan') }}
                        <br>{{ stringlang('structure', 'secara jelas dan instan') }}
                    @endif
                </p>
...
<section class="key-benefit-section" style="padding-bottom: 0;">
    <div class="cloud-container">
        ...
        @if(activelang() == 'KOR')
            <span>통합 회계</span> 기반의 <span>간편</span>하고 <span>효율</span>적인 재무 분석
        @elseif(activelang() == 'ID')
            <span>Akuntansi terintegrasi</span> membantu menganalisis laporan keuangan dengan cara yang <span>sederhana</span> dan <span>efisien</span>
        @else
            Integrated <span>accounting</span> helps analyze financial report in <span>simple and efficient way</span>
        @endif
        </h2>
    </div>
</section>
    ...
    <div class="cloud-container">
        <div class="cloud-diagram-card">
            <div class="cloud-diagram-center">
                @if(activelang() == 'KOR')
                    <img src="{{ asset('assets/img/accounting-integrated-kor.png') }}" alt="">
                @elseif(activelang() == 'ID')
                    <img src="{{ asset('assets/img/accounting-integrated-ind.png') }}" alt="">
                @else
                    <img src="{{ asset('assets/img/accounting-integrated-eng.png') }}" alt="">
                @endif
            </div>
        </div>
    </div>
