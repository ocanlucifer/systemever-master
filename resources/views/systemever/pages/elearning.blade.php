...
<div>
    ...
    <section class="pb-8 pt-0 lg:pb-20">
        <div class="container" style="padding: 0">
            <div class="flex flex-col items-center lg:gap-6 gap-5">
                <h2 class="font-poppins font-bold lg:text-45px text-22px text-center text-[#019641] lg:mb-10 mb-4">{{ $meta->{'image-title3'} }}</h2>
                <div class="flex justify-between w-full gap-10 lg:overflow-visible overflow-auto px-6">
                    @foreach( $meta->{'Learning Steps'} as $usp)
                    <div class="flex flex-col lg:gap-5 gap-3  items-center flex-shrink-0">
                        <img src="{{ $usp->{'icon'} }}" class="lg:h-auto h-66px">
                        <span class="font-opensans lg:text-18px text-8px font-semibold  text-center">{{ $usp->{'title'} }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <section class="relative overflow-hidden" style="height: 360px">
        <img src="{{asset("assets/fl/bg-01.png")}}" class="absolute w-full h-full object-cover top-0 left-0 hidden lg:block">
        <img src="{{asset("assets/fl/bg-02.png")}}" class="absolute w-full h-full object-cover top-0 left-0 lg:hidden block">
        <div class="container relative h-full">
            <div class="flex lg:flex-row flex-col-reverse items-center justify-center h-full">
                <div class="flex flex-col lg:items-start items-center lg:gap-6 gap-3 lg:w-[50%] w-full">
                    <h2 class="font-poppins font-bold lg:text-45px text-22px text-center text-white">{{ $meta->{'title3'} }}</h2>
                    <p class="font-opensans font-semibold lg:text-18px text-12px text-center lg:leading-7 leading-5 text-white ">
                        {{ $meta->{'subtitle3'} }}
                    </p>
                    <a href="{{ $meta->{'title3-button-link'} }}" class="font-montserrat text-white border-white font-bold lg:h-52px h-40px border rounded-full lg:px-12 px-6 lg:text-18px text-12px items-center justify-center  flex transition duration-200 hover:bg-white hover:text-white">{{ $meta->{'title3-button-text'} }}</a>
                </div>
                <div class="h-4 w-full lg:hidden">&nbsp;</div>
                <div class="flex  lg:w-[50%] w-full items-center justify-center">
                    <img src="{{asset("assets/fl/cert-01.svg")}}" class="lg:h-auto h-135px">
                </div>
            </div>
        </div>
    </section>
