<?php namespace App\Http\Controllers;
	...
	class AdminInquiriesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
			$this->col[] = ["label"=>"Date","name"=>"created_at", "callback" => function($row){
				$date = dateformat($row->created_at);
				return '<div style="width:150px">'.$date.'</div>';
			}];
			...
			$this->col[] = ["label"=>"Title","name"=>"title","callback" => function($row){
				return '<div style="width:200px">'.$row->title.'</div>';
			}];
			$this->col[] = ["label"=>"Phone","name"=>"phone"];
			$this->col[] = ["label"=>"Email","name"=>"email"];
			$this->col[] = ["label"=>"Company","name"=>"company"];
			$this->col[] = ["label"=>"Is Experienced Erp","name"=>"is_experienced_erp"];
			$this->col[] = ["label"=>"From","name"=>"from","callback" => function($row){
				return '<div style="width:200px;word-wrap: break-word;">'.$row->from.'</div>';
			}];
			...
			$this->form[] = ['label'=>'Email','name'=>'email','type'=>'email','validation'=>'required|min:1|max:255|email|unique:inquiries','width'=>'col-sm-10','placeholder'=>'Please enter a valid email address'];
			$this->form[] = ['label'=>'Company','name'=>'company','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Is Experienced Erp','name'=>'is_experienced_erp','type'=>'radio','validation'=>'required|integer','width'=>'col-sm-10','dataenum'=>'Array'];
			...
			//$this->form[] = ["label"=>"Email","name"=>"email","type"=>"email","required"=>TRUE,"validation"=>"required|min:1|max:255|email|unique:inquiries","placeholder"=>"Please enter a valid email address"];
			//$this->form[] = ["label"=>"Company","name"=>"company","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Is Experienced Erp","name"=>"is_experienced_erp","type"=>"radio","required"=>TRUE,"validation"=>"required|integer","dataenum"=>"Array"];
			...
			//$this->form[] = ["label"=>"Video Categories Id","name"=>"video_categories_id","type"=>"select2","required"=>TRUE,"validation"=>"required|integer|min:0","datatable"=>"video_categories,name"];
			# OLD END FORM
	        ...
	        */
			...
			$this->index_button[] = [
				'label' => 'Excel Export',
				'url' => CRUDBooster::mainpath() . '/excelexport',
				'icon' => 'fa fa-file',
			];
	    ...
	    */
		...
		{
			$peserta = \App\Inquiries::whereRaw("deleted_at is null")
				->orderBy("id", "desc")
				->get();

			$header = [];

			$header[] = ['key' => 'created_at', 'label' => 'Date'];
			$header[] = ['key' => 'name', 'label' => 'Name'];
			$header[] = ['key' => 'company', 'label' => 'Company'];
			$header[] = ['key' => 'title', 'label' => 'Title in company'];
			$header[] = ['key' => 'phone', 'label' => 'Phone'];
			$header[] = ['key' => 'email', 'label' => 'Email'];
			$header[] = ['key' => 'is_experienced_erp', 'label' => 'Experienced ERP'];
			$header[] = ['key' => 'from', 'label' => 'From'];
			$header[] = ['key' => 'type', 'label' => 'Type'];
			$header[] = ['key' => 'message', 'label' => 'Message'];

			return \App\Service\ExcelService::export($header, $peserta);
		}
