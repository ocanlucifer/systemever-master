<?php namespace App\Http\Controllers;
	...
	class AdminPagesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form = [];
			$this->form[] = ["label"=>"Parent Page","name"=>"parent_id","type"=>"hidden"];
			...
			$this->form[] = ['label'=>'Slug','name'=>'slug','type'=>'text','validation'=>'','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Sort Description','name'=>'body','type'=>'textarea','validation'=>'','width'=>'col-sm-10', 'placeholder' => 'Sort description is needed for SEO'];

			$this->form[] = ['width'=>'col-sm-12','label'=>'','name'=>'json_meta','type'=>'custom','html'=> view("admin/form/custommeta", 
				['editmodule' => 'show']
			)];
			/*
			...
			if (!empty($post_id)) {
				...
				//load the json_meta form
				// dd($post);
				if (!empty($setting) || (!empty($post->json_meta) && $post->json_meta != "null")) {
					$this->form[] = ['width'=>'col-sm-12','label'=>'','name'=>'json_meta','type'=>'custom','html'=> view("admin/form/postmeta", [
							"setting" => $setting, 
							"post" => $post
						])
					];
	        ...
	        */
			...
			// $this->addaction[] = [
			// 	"label" => "SEO Script",
			// 	"url" => CRUDBooster::mainpath() . "/seo/[id]",
			// 	"icon" => "fa fa-globe",
			// 	"color" => "success",
			// ];
	    ...
	    */
	    public function hook_query_index(&$query) {
	        //Your code here
			$query->where('tipe', 'pages');
			$query->whereRaw('(parent_id is null OR parent_id = 0)');
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
	        //Your code here
			$postdata['json_meta'] = json_encode($postdata['json_meta']);
			$postdata['tipe'] = "pages";
	    }
	    ...
	    */
	    public function hook_after_add($id) {        
			...
			$seo = $this->_autogenerate($post);
			// $post->seo = base64_encode($seo);
			$post->save();
	    }
	    ...
	    */
		...
		public function postSeo($id, \Illuminate\Http\Request $request)
		{
			$post = \App\Post::find($id);
			$post->seo = base64_encode($request->seo);
			$post->save();

			return redirect((CRUDBooster::mainpath() . "/seo/" . $id));
		}

		public function getGenerateseo()
		{
			$post = \App\Post::where("tipe", "pages")->get();
			foreach ($post as $v) {
				$seo = $this->_autogenerate($v);
				$v->seo = base64_encode($seo);
				$v->save();
			}
		}

		public function _autogenerate($post)
		{
			$data = [
				"title" => $post->title,
			...
			return view("seogenerator", $data)->render();
		}
